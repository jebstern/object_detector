...
image_lib.Image? convertCameraImage(CameraImage cameraImage) {
...
image_lib.Image convertBGRA8888ToImage(CameraImage cameraImage) {
  final image_lib.Image img = image_lib.Image.fromBytes(cameraImage.planes[0].width!, cameraImage.planes[0].height!, cameraImage.planes[0].bytes, format: image_lib.Format.bgra);
  return img;
}
...
Future<void> saveImage(image_lib.Image image, [int i = 0]) async {
  final List<int> jpeg = image_lib.JpegEncoder().encodeImage(image);
